...
public class PlayAnimationD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Controller.GetPressUp(SteamVR_Controller.ButtonMask.Trigger))//SLeftController.GripRealeasei clic gauche
        {
            if (!firstHit)//si premier tir
            {
                Debug.Log("play ouverture");
                firstHit = true;
                GetComponent<Animation>().Play("OuvertureCouvercleD");
                return;
            }
            if (firstHit && !secondHit)//si second tir
            {
                Debug.Log("play Fermeture");
                GetComponent<Animation>().Play("FermetureCouvercleD");
                secondHit = true;
                return;
            }
        }
